...
| `eshell journal [--unit U] [-n N] [--since EXPR] [--json]` | Tail the systemd journal. Filters compose: `--unit foo --since "1 hour ago"` gives that unit's last hour. Default is the last 50 entries from everything. Priority tag `[E]`/`[W]`/`[N]`/`[D]` is colorized on a TTY (red/yellow/bold/dim). Multi-line messages collapse to one terminal line with `⏎` markers; `--json` preserves the raw message. |
| `eshell modem [-m INDEX] [--no-sim] [--json]` | Modem + primary-SIM details from ModemManager, rendered as two sectioned blocks (`Modem N` / `SIM N`). `State` colored on a TTY (green for connected/registered, yellow for searching/connecting, red for failed/disabled); `Signal` colored by band (green > 75%, yellow 25–75%, red < 25%). Without `-m`, the first modem mmcli reports is used; pass `-m 1` (etc.) on multi-modem devices. `--no-sim` skips the SIM lookup. |
...
| `eshell repl [--no-history]` | Interactive line-by-line REPL. Each line runs through the framed exec protocol, so stdout, stderr, and the exit code stay cleanly separated. Built-ins are prefixed with `\`: `\help`, `\quit`, `\timeout <secs>`, `\cd [dir]` (stateful per-session cwd applied via `Command::cwd()`), `\pwd`, `\reconnect` (re-open the transport and re-activate after a USB unplug / device reboot). Default per-command timeout is 30 s. Ctrl-C during a running command exits the REPL gracefully (saves history, deactivates the shell) instead of getting SIGKILL'd by the OS. History is persisted to `$XDG_STATE_HOME/eshell/history` unless `--no-history`. |
