...
| [`embedded-shell-cli`](crates/embedded-shell-cli) | binary (`eshell`) | Command-line driver built on top of the three libraries. `eshell exec / push / pull / info / ping / reboot / service / services / journal / modem / network / repl / completions`. Useful as a daily-driver tool **and** as a reference application showing how to compose the libraries. |

Each library crate has its own README with full API details:

- [`crates/embedded-shell/README.md`](crates/embedded-shell/README.md) — `Shell` trait, concrete shells, custom prompts, reconnect, event schema, `test-utils` feature
...
```sh
# Develop locally:
cargo build -p embedded-shell-cli              # → target/debug/eshell
cargo build -p embedded-shell-cli --release    # → target/release/eshell
